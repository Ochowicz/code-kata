...
def change_me(money):
    coin_values = {"£5": 500, "£2": 200, "£1": 100, "50p": 50, "20p": 20}
    if money not in coin_values:
        return money
    if money == '20p':
        return '10p 10p'
    change_coins = []
    change = coin_values[money]
    while change >= 20:
        change_coins.append('20p')
        change += -20
    while change >= 10:
        change_coins.append('10p')
        change += -10
    return ' '.join(change_coins)